...
import com.example.fasol.registration.SignInDirections
...
class ProfileAuth : Fragment(R.layout.profile) {
    ...
    override fun onCreateView(
        ...
        name.text = ProfileManager.getCurrentUser().firstName + " " + ProfileManager.getCurrentUser().lastName
        ...
        updateToken.setOnClickListener {
            RetrofitClient.instance.updateAccessToken(RefreshModel(TokenManager.RefreshToken)).enqueue(object :
                Callback<TokenResponce>
            {
                ...
                override fun onResponse(call: Call<TokenResponce>, response: Response<TokenResponce>) {
                    if(response.code() == 200)
                    {
                        ...
                        Toast.makeText(context, "Токен успешно обновлен!", Toast.LENGTH_SHORT).show()
                    }
                    else
                    {
