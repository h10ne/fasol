...
interface Api {
    ...
    @POST("api//basket/")
    @Headers("Content-Type: application/json")
    fun addToBasket(@Header("Authorization") Authorization: String, @Body body:AddToBasketModel):Call<AddToBasketModel>

    @POST("api/basket/change-product-qty/")
    @Headers("Content-Type: application/json")
    fun changeCountBasket(@Header("Authorization") Authorization: String, @Body body:ChangeBasketCountModel):Call<ChangeBasketCountModel>

    @DELETE("api/basket/remove-from-basket/{id}")
    fun deleteFromBasket(@Header("Authorization") Authorization: String, @Path("id") id: Long):Call<Void>
